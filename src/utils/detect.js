 ...
 * @param {Number} scoreThreshold Float representing the threshold for deciding when to remove boxes based on score
 * @param {Number[]} inputShape model input shape. Normally in YOLO model [batch, channels, width, height]
 * @param {Function} onDetectionComplete Callback function called after detection with boxes array
 */
export const detectImage = async (
  image,
  canvas,
  ...
  scoreThreshold,
  inputShape,
  onDetectionComplete = null  // 新增回调参数
) => {
  const ctx = canvas.getContext("2d");
  ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height); // clean canvas
  ...
  // 调用回调函数，传递检测到的 boxes 和原始图像
  if (onDetectionComplete && boxes.length > 0) {
    onDetectionComplete(boxes, image);
  }
